...
export async function createCalendar(req, res) {
  ...
  if (CheckValidator(req, res)) return;
  const { id,title,startDate,endDate,detail } = req.body;
  let result;
  try {
    const sql = `INSERT INTO calendar values(default,'${title}','${startDate}','${endDate}','${detail}',${id});`;
...
export async function getCalendar(req, res) {
  ...
  let result;
  try {
    const sql = `SELECT * FROM calendar WHERE user_id='${id}'`;
    result = await pool.query(sql);
  } catch (e) {
    if (e) return res.json(e.message);
  }

  if(!result[0][0]) return res.status(400).send('해당 ID의 calendar 데이터가 존재하지 않습니다');
...
export async function updateCalendar(req,res){
  ...
  let result;
  try {
    const sql = `UPDATE calendar SET title = '${title}', startDate = '${startDate}',endDate = '${endDate}' ,detail='${detail}' WHERE id = ${id};`;
    result = await pool.query(sql);
  } catch (e) {
    if (e) return res.json(e.message);
  }

  if(result[0].affectedRows == 0) return res.status(400).json({res:'fail',msg:'해당 ID의 calendar가 존재하지 않습니다'});
