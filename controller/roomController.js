...
export async function createRoom(req, res) {
  ...
  if (CheckValidator(req, res)) return;
  const { roomName,max_member } = req.body;
  let result;
  try {
    const sql = `INSERT INTO study_group values(default,'${roomName}',true,${max_member},0,0,0)`;
...
export async function updateRoom(req,res){
  ...
  if(result[0].affectedRows == 0) return res.status(400).json({res:'fail',msg:'해당 ID의 study_group이 존재하지 않습니다'});
